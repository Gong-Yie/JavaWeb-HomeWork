...
<head>
    ...
    <style>
        /* 个人中心专用样式 */
        .profile-wrapper { display: flex; min-height: 500px; }
        .profile-sidebar { width: 250px; background: rgba(0, 0, 0, 0.03); border-right: 1px solid rgba(0, 0, 0, 0.05); padding: 30px 0; display: flex; flex-direction: column; }
        .profile-sidebar ul { list-style: none; padding: 0; margin: 0; }
        .profile-sidebar li { padding: 15px 30px; cursor: pointer; color: #555; font-weight: 500; transition: all 0.3s; border-left: 4px solid transparent; font-size: 15px; }
        .profile-sidebar li:hover { background-color: rgba(255, 255, 255, 0.5); color: #2575fc; }
        .profile-sidebar li.active { background-color: rgba(37, 117, 252, 0.1); color: #2575fc; border-left-color: #2575fc; }
        .profile-content { flex: 1; padding: 40px 50px; text-align: left; }
        .section-title { font-size: 24px; color: #333; margin-bottom: 30px; padding-bottom: 10px; border-bottom: 2px solid #f0f0f0; font-weight: 600; }
        
        .avatar-section { display: flex; align-items: center; margin-bottom: 30px; }
        .big-avatar { width: 100px; height: 100px; border-radius: 50%; border: 4px solid white; box-shadow: 0 5px 15px rgba(0,0,0,0.1); object-fit: cover; cursor: zoom-in; transition: transform 0.3s; }
        .big-avatar:hover { transform: scale(1.05); }
        .avatar-info { margin-left: 20px; }
        
        .info-row { margin-bottom: 15px; font-size: 15px; color: #555; display: flex; align-items: center; }
        .info-label { width: 100px; font-weight: bold; color: #888; }
        .info-value { color: #333; font-weight: 500; }
        
        .form-input { width: 100%; padding: 10px 15px; border: 1px solid #ddd; border-radius: 6px; margin-top: 5px; box-sizing: border-box; background: rgba(255,255,255,0.8); }
            ...
            <div class="profile-wrapper">
                ...
                <div class="profile-content">
                    ...
                    <div id="view-info">
                        ...
                        <div class="info-row"><span class="info-label">登录账号:</span> <span class="info-value">${user}</span></div>
                        <div class="info-row"><span class="info-label">性别:</span> <span class="info-value">${currUser_gender}</span></div>
                        
                        <!-- 仅学生显示班级和学号 -->
                        <% if(!"admin".equals(session.getAttribute("role"))) { %>
                            <div class="info-row"><span class="info-label">所属班级:</span> <span class="info-value">${currUser_class}</span></div>
                            <div class="info-row"><span class="info-label">学号:</span> <span class="info-value">${currUser_no}</span></div>
                        <% } %>
                        
                        <div class="info-row"><span class="info-label">联系电话:</span> <span class="info-value">${empty currUser_phone ? '未填写' : currUser_phone}</span></div>
                        <div class="info-row"><span class="info-label">电子邮箱:</span> <span class="info-value">${empty currUser_email ? '未绑定' : currUser_email}</span></div>
                    ...
                    <div id="view-edit" style="display:none;">
                        ...
                        <form action="user" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label style="font-size:14px; font-weight:bold; color:#555;">姓名 / 昵称</label>
                                <input type="text" name="nickname" value="${currUser_nick}" class="form-input" required>
                            </div>

                            <div class="form-group">
                                <label style="font-size:14px; font-weight:bold; color:#555;">性别</label>
                                <div style="margin-top:5px;">
                                    <label><input type="radio" name="gender" value="男" ${currUser_gender == '男' ? 'checked' : ''}> 男</label>
                                    <label style="margin-left:20px;"><input type="radio" name="gender" value="女" ${currUser_gender == '女' ? 'checked' : ''}> 女</label>
                                </div>
                            </div>

                            <div class="form-group">
                                <label style="font-size:14px; font-weight:bold; color:#555;">联系电话</label>
                                <input type="text" name="phone" value="${currUser_phone}" class="form-input" placeholder="请输入手机号">
                            </div>

                            <div class="form-group">
                                <label style="font-size:14px; font-weight:bold; color:#555;">电子邮箱</label>
                                <input type="email" name="email" value="${currUser_email}" class="form-input" placeholder="例如：example@qq.com">
                            </div>
